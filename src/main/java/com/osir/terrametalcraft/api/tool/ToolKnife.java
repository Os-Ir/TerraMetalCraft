...
public class ToolKnife extends ToolBase implements IGrindstoneTool {
	...
	public boolean matches(ItemStack[] parts) {
		...
		return MaterialUtil.getMaterialTag(ModSolidShapes.KNIFE_HEAD).contains(parts[0].getItem()) && (item2 == Items.STRING || item2 == ModItems.THIN_STRAW_ROPE) && (item3 == Items.STICK) && parts[3].isEmpty() && parts[4].isEmpty();
	}
	...
	public Map<Integer, Pair<String, MaterialBase>> getMaterial(ItemStack[] parts) {
		...
		for (MaterialBase material : MaterialBase.REGISTRY.values()) {
			ITag<Item> tag = MaterialUtil.getMaterialTag(ModSolidShapes.KNIFE_HEAD, material);
